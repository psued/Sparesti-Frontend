  ...
  try {
    ...
    if (response.status === 200 && response.data) {
      ...
      const addRowResponse = await api.post(
        `/budget/budgets/${budgetId}/rows/add`,
        row,
      );
...
export const deleteBudgetRow = async (
  ...
  try {
    const response = await api.delete(
      `/budget/budgets/${budgetId}/rows/${rowId}`,
    );
...
export const renewBudget = async (
...
export const renewBudgetWithCategories = async (
  ...
  try {
    ...
    if (oldBudgetResponse.status !== 200) {
      console.error(
        "Failed to fetch old budget:",
        oldBudgetResponse.statusText,
      );
      return null;
    }
    ...
    for (const category of categories) {
      const addRowResponse = await api.post(
        `/budget/budgets/${newBudgetId}/rows/add`,
        {
          name: category.name,
      ...
      if (addRowResponse.status !== 200) {
        console.error(
          "Failed to add category to new budget:",
          addRowResponse.statusText,
        );
        return null;
      }
...
export const getBudgetWithNewestExpiryDate =
  async (): Promise<Budget | null> => {
    try {
      const response = await api.get(`/budget/budgets`);
      ...
      if (response.status === 200) {
        if (Array.isArray(response.data)) {
          ...
          console.error(
            "Failed to fetch budgets: response data is not an array",
          );
          return null;
        }
      } else {
...
export const addBudgetWithRow = async (
  ...
  for (const row of rows) {
    ...
    if (addRowResponse.status !== 200) {
      throw new Error(
        `Failed to add row to budget: ${addRowResponse.statusText}`,
      );
    }
  }
...
export const useTransactionsNotInBudgetRow = async (): Promise<
  Transaction[] | null
> => {
  try {
    const response = await api.get(
      `/transaction-budget-row/transactions-not-in-budget-row`,
    );
    ...
    if (response.status === 200) {
      ...
      console.error(
        "Failed to fetch transactions not in budget row:",
        response.statusText,
      );
      return null;
    }
...
export const addTransactionToBudgetRow = async (
  budgetRowId: number,
  transactionId: number,
): Promise<boolean> => {
  try {
    const response = await api.post(
      `/transaction-budget-row/add/${budgetRowId}/${transactionId}`,
    );
    ...
    if (response.status === 200) {
      ...
      console.error(
        "Failed to add transaction to budget row:",
        response.statusText,
      );
      return false;
    }
