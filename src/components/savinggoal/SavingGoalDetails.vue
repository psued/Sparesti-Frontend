<template>
  ...
  <div class="saving-goals-container">
    <div
      ...
      class="saving-goal-item"
    >
      <SavingGoalCard
        :savingGoal="goal"
        :editable="false"
        @deleteGoal="handleDelete(Number(goal.id))"
        @editGoal="handleEdit(Number(goal.id))"
        class="saving-goal-card"
        @click="handleRedirect(Number(goal.id))"
      />
    </div>
  </div>
</template>
